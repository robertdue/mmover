...
export class MousePositionWorker implements Worker {
    ...
    private async checkAndMoveIfIdle(): Promise<void> {
        try {
            const currentPosition = await this.mouseController.getPosition();
            ...
            if (this.hasMouseMoved(currentPosition)) {
                ...
                await this.mouseController.moveTo(nextPosition.x, nextPosition.y);
                console.info(`New position: ${nextPosition.x}|${nextPosition.y}`);
            }

            this.lastMousePosition = await this.mouseController.getPosition();
        } catch (error) {
